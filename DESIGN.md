...
The default permission mode is `auto` (until 2026-09-03: `bypassPermissions`), set in Settings and overridable per card. Each node holds its own default, and a run reads the default of the node that runs it. A save in Settings writes a changed default to every node that answers, and the toast names each node that did not take it. The drawer shows the default of the card's node, not the default of the machine that shows the card. `--` ends the options, so a prompt that starts with `-` stays a prompt. Background agents move edits into a git worktree under `.claude/worktrees/`, so parallel runs do not collide. kari records the job id, follows `state.json`, and moves the card: `working` → `validate` on `done`, `needs_approval` on `blocked`, and a notification on `failed`.
