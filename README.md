...
A run starts no MCP server by default. A run is unattended, so a server that needs you is no use to it, and a server that reads the data of another app makes macOS ask the person at the desk for permission, again for every run. Turn the servers on for every card in Settings, and overrule the setting on one card with the MCP servers field in the drawer. kari passes `--strict-mcp-config` with an empty configuration when a run starts none.
...
- A tap on a card opens the card sheet. "Chat" holds the conversation, on the newest turn, and "Details" holds the fields. The prompt box stays at the foot of both. Back closes the sheet, and it also goes to the board from any other tab.
