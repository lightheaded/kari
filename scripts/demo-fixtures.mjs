...
const localCards = [
  ...
  view({
    ...
    session: session(S.cache, projects.docs, {
      ...
      // Markdown, so `bun run demo` shows the drawer's rendered reply.
      last_reply: [
        "I wrote `docs/adr/0007-single-cache-tier.md`. It removes the second tier and keeps the invalidation hooks.",
        "",
        "## What changed",
        "",
        "- One tier, read through, written on miss.",
        "- The invalidation hooks stay where they are.",
        "- The second tier's metrics are gone; nothing read them.",
        "",
        "The read path is now:",
        "",
        "```ts",
        "const hit = await cache.get(key);",
        "return hit ?? (await load(key).then((v) => cache.set(key, v)));",
        "```",
        "",
        "Tell me if you want the diagram in it.",
      ].join("\n"),
      first_at: hours(3),
      last_at: min(6),
      turns: 7,
